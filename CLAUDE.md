...
页面直接读取 Brave News CDN 的 `en_US` feed 和全球 sources 列表，保留新闻列表、频道筛选、发布者分组、自定义来源弹层、刷新重排、隐藏发布者和关注来源覆盖。根域名默认打开“联合早报”，左侧第二个入口保留“为您推荐”。联合早报通过 `/api/zaobao` 展示早报首页中间头图容器和右侧 `aside-realtime`“最新”列表最多 15 条新闻；线上首页初始数据为空时回退解析 `/realtime` 静态列表。缺图项会预取详情页主图。线上环境通过 `/api/image` 代理清洗 Brave CDN 的 `.pad` 图片，本地开发环境直接使用原始图片地址。
