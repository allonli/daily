...
- 根域名默认打开“联合早报”入口；“为您推荐”保留在左侧第二个入口。联合早报头图优先取首页中间头图容器，下方优先取右侧 `aside-realtime`“最新”列表最多 15 条新闻；线上首页初始数据为空时会回退解析 `/realtime` 静态列表。
- 对联合早报缺图新闻会预取详情页，读取结构化数据里的文章主图后再返回给前端。
- 联合早报数据带三层兜底：Vercel CDN 短缓存、函数进程内上一版数据、浏览器 localStorage 上一版数据，避免上游偶发失败时刷新后消失。
...
- `tests/news.test.js`、`tests/zaobao.test.js`：覆盖数据规范化、频道筛选、刷新缓存参数、相对时间格式和联合早报解析补图。
...
- 联合早报会在页面加载时自动请求 `/api/zaobao`；点击右下角刷新按钮会给接口追加缓存参数并触发重新抓取，不需要日常手动更新。
